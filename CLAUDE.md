...
- `views/ArchiveView.vue` / `ArchiveDetailView.vue` — list of past (completed/cancelled) plans and a detail view of one, both fetched straight from the API rather than through a store (see `stores/mealPlan.js` note above). The grocery items themselves are read-only (`GroceryListSection` with `readonly`), but the plan record isn't: both views can delete it, and the detail view can also reuse it — see "Weekly meal planner" below for both
